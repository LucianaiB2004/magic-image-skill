...
又名“幻影坦克 Skill”（Magic Image Skill / Mirage Tank Skill）。它把两层视觉内容编码进**同一张真正的 RGBA PNG**：浅色背景看到正常画面，深色背景显现隐藏画面。

![平静海面与风暴巨浪对比](assets/showcase/comparison.png)

> 默认案例：白底是一艘驶过平静海面的帆船，黑底会显现巨浪、暴雨与闪电。上图只是压平后的左右对比；真正成品是[同一张透明 PNG](assets/showcase/calm-to-storm.png)。
...
一句大白话：同一张透明 PNG，在不同底色上会呈现不同视觉内容。最终效果来自 PNG 的 RGB、Alpha 与背景色的真实合成，不使用图片切换、GIF、视频或 Canvas 偷换。

## 原理

像素显示值为 `RGB × Alpha + Background × (1 - Alpha)`。若白底目标为 `W`、黑底目标为 `K`，理想解满足 `a = 1 - W + K`。RGB 三通道共享同一个 Alpha，因此任意两张彩图不一定有精确解；`pair` 会做白底优先的近似拟合并报告误差，绝不宣称无损转换。
